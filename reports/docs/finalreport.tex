...
Encoders receive only the cleaned review text. Category and rating are retained as evaluation metadata: category is used for clustering, retrieval, and UMAP coloring, while rating is used for the linear probe.
...
\begin{tikzpicture}[
    ...
    \node (tasks) [pipe, right=0.8cm of encoder] {
        ...
        {\small KMeans, FAISS,\\linear probe, efficiency}
    };
...
\caption{End-to-end experimental pipeline. The same cleaned Parquet corpus is encoded by all five representation methods, then reused for clustering, retrieval, visualization, linear probing, and efficiency analysis.}
...
\end{equation}
Architecturally, the probe is deliberately shallow: the frozen review embedding $z_i$ is the input layer, a single affine layer maps it to five logits for star ratings 1--5, and a softmax layer converts those logits into class probabilities. Thus $W \in \mathbb{R}^{5 \times p}$ and $b \in \mathbb{R}^{5}$ are the only trainable parameters, where $p$ is the encoder dimension: 300 for TF-IDF/SVD, Word2Vec, and GloVe, 384 for SBERT, and 1024 for BGE-large. There are no hidden layers, dropout layers, nonlinear projection heads, or encoder fine-tuning; all embedding arrays remain frozen. The implementation uses stochastic-gradient logistic regression with log-loss, L2 penalty, mini-batches of 16,384 examples, and \texttt{partial\_fit} updates, so the classifier is a lightweight probe rather than a high-capacity downstream model. In the full run this gives 1,557,294 training rows and 389,324 held-out test rows. Efficiency analysis records rows, dimensions, file size, device, and parsed embedding runtime.
...
Table~\ref{tab:static-ablation} compares pretrained and corpus-trained Word2Vec/GloVe variants using the same cleaned rows, whitespace-token input, 300-dimensional word vectors, averaged document composition, and downstream evaluation code. This makes the ablation fair for testing the effect of token-vector source under a fixed static document encoder. It is not a data-budget-controlled pretraining benchmark: the pretrained vectors come from large external corpora, while the corpus-trained variants are learned from the full unlabeled Amazon review corpus before the frozen embeddings are evaluated. Therefore, the ablation should be interpreted as external lexical knowledge versus in-domain unsupervised adaptation, not as a strictly train-split-only supervised comparison. Runtime is not used as a direct comparison because the corpus-trained variants were run locally after the original server experiment.
